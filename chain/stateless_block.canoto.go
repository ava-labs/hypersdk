...
func (c *Block) CalculateCanotoCache() {
	...
	if c.BlockContext != nil {
		...
		if fieldSize := (c.BlockContext).CachedCanotoSize(); fieldSize != 0 {
			size += len(canoto__Block__BlockContext__tag) + canoto.SizeUint(uint64(fieldSize)) + fieldSize
		}
	}
	for i := range c.Txs {
		var fieldSize int
		...
		size += len(canoto__Block__Txs__tag) + canoto.SizeUint(uint64(fieldSize)) + fieldSize
	}
	if !canoto.IsZero(c.StateRoot) {
		size += len(canoto__Block__StateRoot__tag) + canoto.SizeBytes((&c.StateRoot)[:])
	}

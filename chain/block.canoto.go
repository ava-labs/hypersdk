...
const (
	...
	canoto__Block__Timestamp__tag    = "\x10" // canoto.Tag(2, canoto.Varint)
	canoto__Block__Height__tag       = "\x19" // canoto.Tag(3, canoto.I64)
	canoto__Block__BlockContext__tag = "\x22" // canoto.Tag(4, canoto.Len)
	canoto__Block__StateRoot__tag    = "\x2a" // canoto.Tag(5, canoto.Len)
	canoto__Block__Txs__tag          = "\x32" // canoto.Tag(6, canoto.Len)
)
...
func (*Block[T1, T2]) MakeCanoto() *Block[T1, T2] {
	return new(Block[T1, T2])
}
...
func (c *Block[T1, T2]) UnmarshalCanoto(bytes []byte) error {
	r := canoto.Reader{
		B: bytes,
	}
...
func (c *Block[T1, T2]) UnmarshalCanotoFrom(r canoto.Reader) error {
	var minField uint32
	for canoto.HasNext(&r) {
		...
		case 2:
			...
			if err := canoto.ReadSint(&r, &c.Timestamp); err != nil {
				return err
			}
		...
		case 3:
			...
			if err := canoto.ReadFint64(&r, &c.Height); err != nil {
				return err
			}
		...
		case 4:
			...
			remainingBytes := r.B
			r.B = msgBytes
			c.BlockContext = c.BlockContext.MakeCanoto()
...
func (c *Block[T1, T2]) ValidCanoto() bool {
	if c == nil {
		return true
	}
	if !c.BlockContext.ValidCanoto() {
		return false
	}
	for i := range c.Txs {
...
func (c *Block[T1, T2]) CalculateCanotoCache() {
	if c == nil {
		return
	}
	c.canotoData.size = 0
	...
	if !canoto.IsZero(c.Timestamp) {
		c.canotoData.size += len(canoto__Block__Timestamp__tag) + canoto.SizeSint(c.Timestamp)
	}
	if !canoto.IsZero(c.Height) {
		c.canotoData.size += len(canoto__Block__Height__tag) + canoto.SizeFint64
	}
...
func (c *Block[T1, T2]) CachedCanotoSize() int {
	if c == nil {
		return 0
	}
...
//
// It is not safe to call this function concurrently.
func (c *Block[T1, T2]) MarshalCanoto() []byte {
...
//
// It is not safe to call this function concurrently.
func (c *Block[T1, T2]) MarshalCanotoInto(w canoto.Writer) canoto.Writer {
	if c == nil {
		return w
	}
	if !canoto.IsZero(c.ParentID) {
	...
	if !canoto.IsZero(c.Timestamp) {
		...
		canoto.AppendSint(&w, c.Timestamp)
	}
	if !canoto.IsZero(c.Height) {
		canoto.Append(&w, canoto__Block__Height__tag)
		canoto.AppendFint64(&w, c.Height)
	}
	if fieldSize := c.BlockContext.CachedCanotoSize(); fieldSize != 0 {

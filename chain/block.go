...
func (b *StatefulBlock) innerVerify(ctx context.Context, vctx VerifyContext) error {
	...
	heightKey := b.vm.StateLayout().HeightPrefix()
	...
	timestampKey := b.vm.StateLayout().TimestampPrefix()
	...
	feeKey := b.vm.StateLayout().FeePrefix()
...
func (b *StatefulBlock) View(ctx context.Context, verify bool) (state.View, error) {
	...
	if b.accepted {
		...
		acceptedHeightRaw, err := acceptedState.Get(b.vm.StateLayout().HeightPrefix())
		if err != nil {
			return nil, err
		}

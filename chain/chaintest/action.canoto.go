...
// versions:
// 	canoto v0.10.0
// source: chain/chaintest/action.go
...
func (c *TestAction) CalculateCanotoCache() {
	...
	if !canoto.IsZero(c.Start) {
		c.canotoData.size += len(canoto__TestAction__Start__tag) + canoto.SizeSint(c.Start)
	}
	if !canoto.IsZero(c.End) {
		c.canotoData.size += len(canoto__TestAction__End__tag) + canoto.SizeSint(c.End)
	}
}

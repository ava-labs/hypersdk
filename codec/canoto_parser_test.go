...
func TestTypeParser(t *testing.T) {
	cp := NewCanotoParser[Blah]()

	t.Run("empty parser", func(t *testing.T) {
		require := require.New(t)
		f, ok := cp.lookupIndex(0)
		require.Nil(f)
		require.False(ok)
	})
	...
	t.Run("populated parser", func(t *testing.T) {
		...
		require.NoError(
			cp.Register(blah2, func([]byte) (Blah, error) { return nil, errBlah2 }),
		)
		...
		f, ok = cp.lookupIndex(blah2.GetTypeID())
		require.True(ok)
		res, err = f(nil)
		require.Nil(res)

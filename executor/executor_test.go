...
func TestExecutorNoConflicts(t *testing.T) {
	...
	for i := 0; i < 100; i++ {
		...
		for k := 0; k < i+1; k++ {
			s.Add(ids.GenerateTestID().String(), state.Permission(state.Read, state.Write))
		}
		ti := i
		e.Run(s, func() error {
			l.Lock()
...
func TestExecutorSimpleConflict(t *testing.T) {
	...
	for i := 0; i < 100; i++ {
		...
		if i%10 == 0 {
			s.Add(conflictKey, state.Permission(state.Read, state.Write))
		}
		ti := i
		e.Run(s, func() error {
			if ti == 0 {
...
func TestExecutorMultiConflict(t *testing.T) {
	...
	for i := 0; i < 100; i++ {
		...
		if i == 15 || i == 20 {
			s.Add(conflictKey2, state.Permission(state.Read, state.Write))
		}
		ti := i
		e.Run(s, func() error {
			if ti == 0 {

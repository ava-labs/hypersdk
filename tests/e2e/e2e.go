...
var _ = ginkgo.Describe("[Test]", func() {
	...
	ginkgo.It("start network", func() {
		...
		for _, nodeName := range subnet {
			...
			instances = append(instances, instance{
				...
				uri:    u,
				cli:    cli,
				lcli:   vmBackend.NewJSONRPCClient(u, networkID, bid),
			})
		}
	})
	...
	ginkgo.It("transfer in a single node (raw)", func() {
		...
		ginkgo.By("issue Transfer to the first node", func() {
			...
			action0 := vmBackend.NextAction()
			submit, tx, _, err := instances[0].cli.GenerateTransaction(
				context.Background(),
				parser,
				[]chain.Action{action0},
				vmBackend.AuthFactory(),
			)
			require.NoError(err)
	...
	ginkgo.It("supports issuance of 128 blocks", func() {
		count += generateBlocks(context.Background(), vmBackend, count, 128, instances, true)
	})
	...
	ginkgo.It("can bootstrap a new node", func() {
		...
		require.NoError(err)
		tc := vmBackend.NewJSONRPCClient(uri, networkID, bid)
		lsyncClient = tc
		instances = append(instances, instance{
			uri:  uri,
			cli:  c,
	...
	ginkgo.It("accepts transaction after it bootstraps", func() {
		acceptTransaction(vmBackend, syncClient, lsyncClient)
	})
	...
	ginkgo.It("supports issuance of at least 1024 more blocks", func() {
		count += generateBlocks(context.Background(), vmBackend, count, 1024, instances, true)
	...
	ginkgo.It("can state sync a new node when no new blocks are being produced", func() {
		...
		networkID, _, _, err := syncClient.Network(context.TODO())
		require.NoError(err)
		lsyncClient = vmBackend.NewJSONRPCClient(uri, networkID, bid)
	})
	...
	ginkgo.It("supports issuance of 256 more blocks", func() {
		count += generateBlocks(context.Background(), vmBackend, count, 256, instances, true)
		// TODO: verify all roots are equal
	})
	...
	ginkgo.It("state sync while broadcasting transactions", func() {
		...
		go func() {
			...
			count += generateBlocks(ctx, vmBackend, count, 0, instances, false)
		}()
...
func generateBlocks(
	...
	for ctx.Err() == nil {
		...
		submit, _, _, err := instances[cumulativeTxs%len(instances)].cli.GenerateTransaction(
			context.Background(),
			parser,
			[]chain.Action{backend.NextAction()},
			backend.AuthFactory(),
		)
		if failOnError {
			require.NoError(err)

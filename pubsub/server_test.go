...
func dummyProcessTXCallback(b []byte, _ *Connection, e []interface{}) []byte {
	if e != nil {
		count := e[0].(*int)
		*count++
	}
...
func TestServerPublish(t *testing.T) {
	...
	server.lock.Lock()
	server.Publish(dummyMsg, &server.conns)
	server.lock.Unlock()
	...
	require.NoError(err, "Error unmarshalling message.")
	// Verify that the received message is the expected dummy message
	require.Equal(dummyMsg, unmarshal, "Response from server not correct.")

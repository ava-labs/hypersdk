...
func (*BatchMessage) CanotoSpec(types ...reflect.Type) *canoto.Spec {
	types = append(types, reflect.TypeOf(BatchMessage{}))
	s := &canoto.Spec{
		Name: "BatchMessage",
...
func (c *BatchMessage) CalculateCanotoCache() {
	...
	for _, v := range c.Messages {
		size += len(canoto__BatchMessage__Messages__tag) + canoto.SizeBytes(v)
	}

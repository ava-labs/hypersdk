...
func (*BlockFetchRequest) CanotoSpec(types ...reflect.Type) *canoto.Spec {
	types = append(types, reflect.TypeOf(BlockFetchRequest{}))
...
func (c *BlockFetchRequest) CalculateCanotoCache() {
	...
	if !canoto.IsZero(c.BlockHeight) {
		size += len(canoto__BlockFetchRequest__BlockHeight__tag) + canoto.SizeFint64
	}
	if !canoto.IsZero(c.MinTimestamp) {
		size += len(canoto__BlockFetchRequest__MinTimestamp__tag) + canoto.SizeFint64
	}
...
// CanotoSpec returns the specification of this canoto message.
func (*BlockFetchResponse) CanotoSpec(types ...reflect.Type) *canoto.Spec {
	types = append(types, reflect.TypeOf(BlockFetchResponse{}))
	s := &canoto.Spec{
...
func (c *BlockFetchResponse) CalculateCanotoCache() {
	...
	for _, v := range c.Blocks {
		size += len(canoto__BlockFetchResponse__Blocks__tag) + canoto.SizeBytes(v)
	}
	c.canotoData.size.Store(int64(size))
}

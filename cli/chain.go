...
func (h *Handler) WatchChain(hideTxs bool, getParser func(string, uint32, ids.ID) (chain.Parser, error), handleTx func(*chain.Transaction, *chain.Result)) error {
	...
	for ctx.Err() == nil {
		...
		if lastBlock != 0 {
			...
			utils.Outf(
				"{{green}}height:{{/}}%d {{green}}txs:{{/}}%d {{green}}root:{{/}}%s {{green}}size:{{/}}%.2fKB {{green}}unit prices:{{/}} [%s] [{{green}}TPS:{{/}}%.2f {{green}}latency:{{/}}%dms {{green}}gap:{{/}}%dms]\n",
				blk.Hght,
				len(blk.Txs),
				blk.StateRoot,
			...
			utils.Outf(
				"{{green}}height:{{/}}%d {{green}}txs:{{/}}%d {{green}}root:{{/}}%s {{green}}size:{{/}}%.2fKB {{green}}unit prices:{{/}} [%s]\n",
				blk.Hght,
				len(blk.Txs),
				blk.StateRoot,
				float64(blk.Size())/units.KiB,

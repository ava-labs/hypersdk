...
func NewSnowVM(
	...
	authEngine map[uint8]vm.AuthEngine,
	options ...vm.Option,
) (*snow.SnowVM[*chain.ExecutionBlock, *chain.OutputBlock, *chain.OutputBlock], error) {
	options = append(options, NewDefaultOptions()...)
	hyperVM, err := New(
		genesisFactory,
		balanceHandler,
	...
	return snow.NewSnowVM[*chain.ExecutionBlock, *chain.OutputBlock, *chain.OutputBlock](v.String(), hyperVM), nil
}

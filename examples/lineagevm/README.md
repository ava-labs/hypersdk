<p align="center">
  <img width="90%" alt="lineagevm" src="assets/logo.jpeg">
</p>
<p align="center">
  The Choice is Yours
</p>
...
The [`lineagevm`](./examples/lineagevm) provides the first glimpse into the world of the `hypersdk`.
...
When you are ready to build your own `hypervm`, we recommend using the `lineagevm` as a template!

## Status

`lineagevm` is considered **ALPHA** software and is not safe to use in
...
The first step to running this demo is to launch your own `lineagevm` Subnet. You
...
To make it easy to interact with the `lineagevm`, we implemented the `morpheus-cli`.

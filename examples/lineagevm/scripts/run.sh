...
if [[ -z "${GENESIS_PATH}" ]]; then
  ...
  --genesis-file "${TMPDIR}"/lineagevm.genesis
else
  echo "copying custom genesis file"
  rm -f "${TMPDIR}"/lineagevm.genesis
  cp "${GENESIS_PATH}" "${TMPDIR}"/lineagevm.genesis
fi
...
rm -f "${TMPDIR}"/lineagevm.config
rm -rf "${TMPDIR}"/lineagevm-e2e-profiles
cat <<EOF > "${TMPDIR}"/lineagevm.config
{
  ...
  "logLevel": "${LOG_LEVEL}",
  "continuousProfilerDir":"${TMPDIR}/lineagevm-e2e-profiles/*",
  "stateSyncServerDelay": ${STATESYNC_DELAY}
}
EOF
mkdir -p "${TMPDIR}"/lineagevm-e2e-profiles
...
rm -f "${TMPDIR}"/lineagevm.subnet
cat <<EOF > "${TMPDIR}"/lineagevm.subnet
{
...
--vm-genesis-path="${TMPDIR}"/lineagevm.genesis \
--vm-config-path="${TMPDIR}"/lineagevm.config \
--subnet-config-path="${TMPDIR}"/lineagevm.subnet \

...
func (cli *JSONRPCClient) GetProfessorDetails(ctx context.Context, professorID string) (string, uint16, string, []codec.Address, error) {
	resp := new(GetProfessorDetailsReply)
	err := cli.requester.SendRequest(
		ctx,
		"getProfessorDetails",
		&GetProfessorDetailsArgs{
			ProfessorID: professorID,
		},
		resp,
	)
	return resp.Name, resp.Year, resp.University, resp.Students, err
}

func (cli *JSONRPCClient) DoesLineageExist(ctx context.Context, professorName string, studentName string) (bool, error) {
	resp := new(LineageCheckReply)
	err := cli.requester.SendRequest(
		ctx,
		"lineageCheck",
		&LineageCheckArgs{

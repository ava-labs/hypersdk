...
# Construct the correct path to aizelvm directory
MORPHEUSVM_PATH=$(
...
if [[ $# -eq 1 ]]; then
    ...
    echo "Invalid arguments to build aizelvm. Requires zero (default location) or one argument to specify binary location."
    exit 1
fi

cd "$MORPHEUSVM_PATH"
mkdir -p "$(dirname "$BINARY_PATH")"
go build -o "$BINARY_PATH" ./cmd/aizelvm

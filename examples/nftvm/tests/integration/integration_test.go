...
import (
	...
	"github.com/ava-labs/hypersdk/examples/nftvm/auth"
	...
	lconsts "github.com/ava-labs/hypersdk/examples/nftvm/consts"
	lrpc "github.com/ava-labs/hypersdk/examples/nftvm/rpc"
...
func TestIntegration(t *testing.T) {
	ginkgo.RunSpecs(t, "nftvm integration test suites")
}
...
var _ = ginkgo.Describe("[Tx Processing]", func() {
	...
	// Testing NFT-VM Specific Logic

	ginkgo.It("Stores NFT Collections", func() {
		ginkgo.By("Sending TX", func() {
			parser, err := instances[0].lcli.Parser(context.Background())
			require.NoError(err)
	...
	ginkgo.It("Stores NFT Instances", func() {
		ginkgo.By("Sending a TX", func() {
			parser, err := instances[0].lcli.Parser(context.Background())
			require.NoError(err)

...
ADDRESS=${ADDRESS:-nft1qrzvk4zlwj9zsacqgtufx7zvapd3quufqpxk5rsdd4633m4wz2fdjk97rwu}
if [[ ${MODE} != "run" ]]; then
  LOG_LEVEL=DEBUG
...
echo "building nft-cli"
go build -v -o "${TMPDIR}"/nft-cli ./cmd/nft-cli
...
if [[ -z "${GENESIS_PATH}" ]]; then
  ...
  rm -f "${TMPDIR}"/nftvm.genesis
  "${TMPDIR}"/nft-cli genesis generate "${TMPDIR}"/allocations.json \
  --window-target-units "${WINDOW_TARGET_UNITS}" \
  --max-block-units "${MAX_BLOCK_UNITS}" \
  --min-block-gap "${MIN_BLOCK_GAP}" \
  --genesis-file "${TMPDIR}"/nftvm.genesis
else
  echo "copying custom genesis file"
  rm -f "${TMPDIR}"/nftvm.genesis
  cp "${GENESIS_PATH}" "${TMPDIR}"/nftvm.genesis
fi
...
rm -f "${TMPDIR}"/nftvm.config
rm -rf "${TMPDIR}"/nftvm-e2e-profiles
cat <<EOF > "${TMPDIR}"/nftvm.config
{
  ...
  "logLevel": "${LOG_LEVEL}",
  "continuousProfilerDir":"${TMPDIR}/nftvm-e2e-profiles/*",
  "stateSyncServerDelay": ${STATESYNC_DELAY}
}
EOF
mkdir -p "${TMPDIR}"/nftvm-e2e-profiles
...
rm -f "${TMPDIR}"/nftvm.subnet
cat <<EOF > "${TMPDIR}"/nftvm.subnet
{
...
--vm-genesis-path="${TMPDIR}"/nftvm.genesis \
--vm-config-path="${TMPDIR}"/nftvm.config \
--subnet-config-path="${TMPDIR}"/nftvm.subnet \

## Status
`programsvm` is considered **ALPHA** software and is not safe to use in
...
## Demo
### Launch Subnet
The first step to running this demo is to launch your own `programsvm` Subnet. You
...
_By default, this allocates all funds on the network to `programs1qrzvk4zlwj9zsacqgtufx7zvapd3quufqpxk5rsdd4633m4wz2fdjk97rwu`. The private
...
### Build `programs-cli`
To make it easy to interact with the `programsvm`, we implemented the `programs-cli`.
Next, you'll need to build this tool. You can use the following command:
```bash
./scripts/build.sh
```

_This command will put the compiled CLI in `./build/programs-cli`._

### Configure `programs-cli`
Next, you'll need to add the chains you created and the default key to the
`programs-cli`. You can use the following commands from this location to do so:
```bash
./build/programs-cli key import ed25519 demo.pk
```

If the key is added corretcly, you'll see the following log:
```
database: .programs-cli
imported address: programs1qrzvk4zlwj9zsacqgtufx7zvapd3quufqpxk5rsdd4633m4wz2fdjk97rwu
```

Next, you'll need to store the URLs of the nodes running on your Subnet:
```bash
./build/programs-cli chain import-anr
```

If `programs-cli` is able to connect to ANR, it will emit the following logs:
```
database: .programs-cli
...
_`./build/programs-cli chain import-anr` connects to the Avalanche Network Runner server running in
...
```bash
./build/programs-cli key balance
```

If successful, the balance response should look like this:
```
database: .programs-cli
address:programs1qrzvk4zlwj9zsacqgtufx7zvapd3quufqpxk5rsdd4633m4wz2fdjk97rwu
...
```bash
./build/programs-cli key generate secp256r1
```

If successful, the `programs-cli` will emit the new address:
```
database: .programs-cli
created address: programs1q8rc050907hx39vfejpawjydmwe6uujw0njx9s6skzdpp3cm2he5s036p07
```

By default, the `programs-cli` sets newly generated addresses to be the default. We run
the following command to set it back to `demo.pk`:
```bash
./build/programs-cli key set
```
...
chainID: 2mQy8Q9Af9dtZvVM8pKsh2rB3cT3QNLjghpet5Mm5db4N7Hwgk
stored keys: 2
0) address (ed25519): programs1qrzvk4zlwj9zsacqgtufx7zvapd3quufqpxk5rsdd4633m4wz2fdjk97rwu balance: 10000000000.000000000 RED
1) address (secp256r1): programs1q8rc050907hx39vfejpawjydmwe6uujw0njx9s6skzdpp3cm2he5s036p07 balance: 0.000000000 RED
set default key: 0
```
...
```bash
./build/programs-cli action transfer
```

The `programs-cli` will emit the following logs when the transfer is successful:
```
database: .programs-cli
address: programs1qqds2l0ryq5hc2ddps04384zz6rfeuvn3kyvn77hp4n5sv3ahuh6wgkt57y
chainID: 2mQy8Q9Af9dtZvVM8pKsh2rB3cT3QNLjghpet5Mm5db4N7Hwgk
balance: 1000.000000000 RED
recipient: programs1q8rc050907hx39vfejpawjydmwe6uujw0njx9s6skzdpp3cm2he5s036p07
✔ amount: 10
continue (y/n): y
...
`programs-cli` comes bundled with a simple explorer that logs all blocks/txs that
occur on-chain. You can run this utility by running the following command from
this location:
```bash
./build/programs-cli chain watch
```
...
✅ sceRdaoqu2AAyLdHCdQkENZaXngGjRoc8nFdGyG8D9pCbTjbk actor: programs1qrzvk4zlwj9zsacqgtufx7zvapd3quufqpxk5rsdd4633m4wz2fdjk97rwu units: 440 summary (*actions.Transfer): [10.000000000 RED -> programs1q8rc050907hx39vfejpawjydmwe6uujw0njx9s6skzdpp3cm2he5s036p07]
```

...
cd avalanche-cli
git checkout ea273893e6d33933257a26fa76349c30695efd76
./scripts/build.sh
...
$TMPDIR/avalanche node loadtest start "default" ${CLUSTER} ${VMID} --region eu-west-1 --aws --node-type c7gn.8xlarge --loadTestRepoURL="https://github.com/ava-labs/hypersdk/commit/${VM_COMMIT}" --loadTestBuildCmd="cd /home/ubuntu/hypersdk/examples/morpheusvm; CGO_CFLAGS=\"-O -D__BLST_PORTABLE__\" go build -o ~/simulator ./cmd/morpheus-cli" --loadTestCmd="/home/ubuntu/simulator spam run ed25519 --accounts=10000 --txs-per-second=100000 --step-size=1000 --conns-per-host=10 --cluster-info=/home/ubuntu/clusterInfo.yaml --private-key=323b1d8f4eed5f0da9da93071b034f2dce9d2d22692c172f3cb252a64ddfafd01b057de320297c29ad0c1f589ea216869cf1938d88c9fbd70d6748323dbf2fa7"
echo -e "${YELLOW}To stop load test, run:${NC} \"${TMPDIR}/avalanche node loadtest stop ${CLUSTER} --load-test=\"default\"\""
echo -e "${YELLOW}To monitor load test, visit grafana dashboard (URL logged above)${NC}"

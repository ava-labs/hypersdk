...
const FillOrder = ({ order }) => {
    ...
    const fillOrder = () => {
      ...
      (async () => {
        try {
          ...
          await FOrder(order.ID, order.Owner, order.InID, order.InputStep, order.OutID, amount.current);
    ...
    return (<>
      <InputNumber placeholder={`Amount (${order.InSymbol})`} stringMode="true" style={{ width: "80%", margin: "8px 4px 0 0" }} onChange={valueUpdate} value={amount.current}/>
      <Button type="primary" onClick={fillOrder}>Fill</Button>
    </>);
}

...
var watchChainCmd = &cobra.Command{
	...
	RunE: func(_ *cobra.Command, args []string) error {
		...
		for ctx.Err() == nil {
			blk, txBlks, results, err := scli.ListenBlock(ctx, parser)
			if err != nil {
				return err
			}
			totalTxs := 0
			for _, txBlk := range txBlks {
				totalTxs += len(txBlk.Txs)
			}
			now := time.Now()
			if start.IsZero() {
				start = now
			...
			if lastBlock != 0 {
				...
				tpsWindow = newWindow
				window.Update(&tpsWindow, window.WindowSliceSize-hconsts.Uint64Len, uint64(totalTxs))

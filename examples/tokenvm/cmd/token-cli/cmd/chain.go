	...
	RunE: func(_ *cobra.Command, args []string) error {
		...
		for ctx.Err() == nil {
			...
			if lastBlock != 0 {
				...
				tpsWindow = newWindow
				window.Update(&tpsWindow, window.WindowSliceSize-hconsts.Uint64Len, uint64(len(txs)))
				runningDuration := time.Since(start)
				tpsDivisor := math.Min(window.WindowSize, runningDuration.Seconds())
				utils.Outf(
					"{{green}}height:{{/}}%d {{green}}chunks:{{/}}%d {{green}}txs:{{/}}%d {{green}}units:{{/}}%d {{green}}root:{{/}}%s {{green}}TPS:{{/}}%.2f {{green}}split:{{/}}%dms\n", //nolint:lll
					blk.Hght,
					len(blk.Chunks),
					len(txs),
					blk.UnitsConsumed,
				...
				utils.Outf(
					"{{green}}height:{{/}}%d {{green}}chunks:{{/}}%d {{green}}txs:{{/}}%d {{green}}units:{{/}}%d {{green}}root:{{/}}%s\n", //nolint:lll
					blk.Hght,
					len(blk.Chunks),
					len(txs),
					blk.UnitsConsumed,

...
func main() {
	...
	if len(c.PrivateKeyBytes) == 0 {
		...
		log.Info("created new faucet address", zap.String("address", codec.MustAddressBech32(consts.HRP, auth.NewED25519Address(priv.PublicKey()))))
	} else {
		log.Info("loaded faucet address", zap.String("address", codec.MustAddressBech32(consts.HRP, auth.NewED25519Address(c.PrivateKey().PublicKey()))))
	}

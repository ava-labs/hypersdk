...
func (f *FillOrder) StateKeys(actor codec.Address, _ ids.ID) state.Keys {
	return state.Keys{
		string(storage.OrderKey(f.Order)):         state.Permission(state.Read, state.Write),
		string(storage.BalanceKey(f.Owner, f.In)): state.Permission(state.Read, state.Write),
		string(storage.BalanceKey(actor, f.In)):   state.Permission(state.Read, state.Write),
		string(storage.BalanceKey(actor, f.Out)):  state.Permission(state.Read, state.Write),
	}
}

...
func (f *FillOrder) StateKeys(actor codec.Address, _ ids.ID) state.Keys {
	return state.Keys{
		string(storage.OrderKey(f.Order)):         state.NewKey(state.Read, state.Write),
		string(storage.BalanceKey(f.Owner, f.In)): state.NewKey(state.Read, state.Write),
		string(storage.BalanceKey(actor, f.In)):   state.NewKey(state.Read, state.Write),
		string(storage.BalanceKey(actor, f.Out)):  state.NewKey(state.Read, state.Write),
	}
}

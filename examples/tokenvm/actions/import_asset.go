...
func (i *ImportAsset) StateKeys(actor codec.Address, _ ids.ID) state.Keys {
	...
	if i.warpTransfer.Return {
		...
		keys = state.Keys{
			string(storage.AssetKey(i.warpTransfer.Asset)):                             &state.Permissions{Read: true, Write: true},
			string(storage.LoanKey(i.warpTransfer.Asset, i.warpMessage.SourceChainID)): &state.Permissions{Read: true, Write: true},
			string(storage.BalanceKey(i.warpTransfer.To, i.warpTransfer.Asset)):        &state.Permissions{Read: true, Write: true},
		}
	} else {
		assetID = ImportedAssetID(i.warpTransfer.Asset, i.warpMessage.SourceChainID)
		keys = state.Keys{
			string(storage.AssetKey(assetID)):                      &state.Permissions{Read: true, Write: true},
			string(storage.BalanceKey(i.warpTransfer.To, assetID)): &state.Permissions{Read: true, Write: true},
		}
	}
	...
	if i.warpTransfer.Reward > 0 {
		keys.Add(string(storage.BalanceKey(actor, assetID)), &state.Permissions{Read: true, Write: true})
	}
	...
	if i.Fill && i.warpTransfer.SwapIn > 0 {
		keys.Add(string(storage.BalanceKey(actor, i.warpTransfer.AssetOut)), &state.Permissions{Read: true, Write: true})
		keys.Add(string(storage.BalanceKey(actor, assetID)), &state.Permissions{Read: true, Write: true})
		keys.Add(string(storage.BalanceKey(i.warpTransfer.To, i.warpTransfer.AssetOut)), &state.Permissions{Read: true, Write: true})
	}
	return keys
}

...
func (b *BurnAsset) StateKeys(actor codec.Address, _ ids.ID) state.Keys {
	return state.Keys{
		string(storage.AssetKey(b.Asset)):          state.Permission(state.Read, state.Write),
		string(storage.BalanceKey(actor, b.Asset)): state.Permission(state.Read, state.Write),
	}
}

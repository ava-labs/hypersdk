...
func (e *EthRPCServer) BlockNumber(req *http.Request, _ *struct{}, reply *hexutil.Uint64) error {
	...
	latestBlock := e.vm.LastAcceptedBlockResult()
	*reply = hexutil.Uint64(latestBlock.Block.Hght)
	e.vm.Logger().Info("eth_blockNumber returning", zap.Any("blockNumber", latestBlock.Block.Hght))
	return nil
}
...
func (e *EthRPCServer) GetBlockByNumber(req *http.Request, args *GetBlockByNumberArgs, reply *map[string]interface{}) error {
	...
	if args.BlockNumber.Int64() < 0 {
		...
		block := e.vm.LastAcceptedBlockResult()
		height = block.Block.Hght
	} else {
	...
	blockInfo["number"] = (*hexutil.Big)(big.NewInt(int64(blk.Block.Hght)))

	blockID := blk.Block.ID()
	blockInfo["hash"] = common.BytesToHash(blockID[:])
	parentID := blk.Block.Prnt
	...
	blockInfo["timestamp"] = hexutil.Uint64(blk.Block.Tmstmp)
...
func (e *EthRPCServer) SendRawTransaction(req *http.Request, args *hexutil.Bytes, reply *common.Hash) error {
	...
	action, err := convertTxToAction(tx, block.Block.Hght, now)
	if err != nil {
...
func (e *EthRPCServer) GetTransactionReceipt(req *http.Request, args *common.Hash, reply *map[string]interface{}) error {
	...
	txInfo["type"] = "0x00"
	txInfo["blockNumber"] = hexutil.Uint64(blk.Block.Hght)
...
func (e *EthRPCServer) simulateAction(ctx context.Context, action chain.Action) (codec.Bytes, state.Keys, error) {
	...
	return output, recorder.GetStateKeys(), nil
}

...
type Indexer struct {
	...
	txIDToHeight *cache.FIFO[ids.ID, uint64]
}
...
func NewIndexer(path string, parser chain.Parser, blockWindow uint64) (*Indexer, error) {
	...
	txIDToHeight, err := cache.NewFIFO[ids.ID, uint64](int(blockWindow))
	if err != nil {
		return nil, err
	}
	i := &Indexer{
...
func (i *Indexer) storeTransactions(blk *chain.ExecutedBlock) error {
	...
	for j, tx := range blk.Block.Txs {
		...
		i.txIDToHeight.Put(tx.GetID(), blk.Block.Hght)
	}
...
func (i *Indexer) GetTransactionHeight(txID ids.ID) (uint64, bool) {
	return i.txIDToHeight.Get(txID)
}

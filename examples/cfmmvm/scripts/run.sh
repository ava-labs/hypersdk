...
if [[ -z "${GENESIS_PATH}" ]]; then
  ...
  --genesis-file "${TMPDIR}"/cfmmvm.genesis
else
  echo "copying custom genesis file"
  rm -f "${TMPDIR}"/cfmmvm.genesis
  cp "${GENESIS_PATH}" "${TMPDIR}"/cfmmvm.genesis
fi
...
rm -f "${TMPDIR}"/cfmmvm.config
rm -rf "${TMPDIR}"/cfmmvm-e2e-profiles
cat <<EOF > "${TMPDIR}"/cfmmvm.config
{
  ...
  "logLevel": "${LOG_LEVEL}",
  "continuousProfilerDir":"${TMPDIR}/cfmmvm-e2e-profiles/*",
  "stateSyncServerDelay": ${STATESYNC_DELAY}
}
EOF
mkdir -p "${TMPDIR}"/cfmmvm-e2e-profiles
...
rm -f "${TMPDIR}"/cfmmvm.subnet
cat <<EOF > "${TMPDIR}"/cfmmvm.subnet
{
...
--vm-genesis-path="${TMPDIR}"/cfmmvm.genesis \
--vm-config-path="${TMPDIR}"/cfmmvm.config \
--subnet-config-path="${TMPDIR}"/cfmmvm.subnet \

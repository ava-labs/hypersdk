<p align="center">
  <img width="90%" alt="cfmmvm" src="assets/logo.jpeg">
</p>
<p align="center">
  The Choice is Yours
</p>
...
The [`cfmmvm`](./examples/cfmmvm) provides the first glimpse into the world of the `hypersdk`.
...
When you are ready to build your own `hypervm`, we recommend using the `cfmmvm` as a template!

## Status

`cfmmvm` is considered **ALPHA** software and is not safe to use in
...
The first step to running this demo is to launch your own `cfmmvm` Subnet. You
...
To make it easy to interact with the `cfmmvm`, we implemented the `morpheus-cli`.

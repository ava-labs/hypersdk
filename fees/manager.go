...
func (f *Manager) ComputeNext(lastTime int64, currTime int64, m Metadata) (*Manager, error) {
	f.l.RLock()
	defer f.l.RUnlock()

	targetUnits := m.GetWindowTargetUnits()
	unitPriceChangeDenom := m.GetUnitPriceChangeDenominator()
	minUnitPrice := m.GetMinUnitPrice()

...
// Commenting this out for now because it could be the case that we remove this in the future.
...
// 			require.NoError(New(0).NewView(stateKeys, immutableScopeStorage(existingKeyValue).duplicate(), blockHeight).Insert(context.Background(), key, []byte{1, 2, 3}))
...
// 			// validate operation agaist TStateView
// 			stateKeys := recorder.GetStateKeys()
// 			require.NoError(New(0).NewView(stateKeys, immutableScopeStorage(existingKeyValue).duplicate(), blockHeight).Insert(context.Background(), key, []byte{1, 2, 3, 4}))
...
// 			// validate operation agaist TStateView
// 			stateKeys := recorder.GetStateKeys()
// 			require.NoError(New(0).NewView(stateKeys, immutableScopeStorage(existingKeyValue).duplicate(), blockHeight).Remove(context.Background(), existingKeys[keyIdx]))
...
// 			// validate operation agaist TStateView
// 			stateKeys := recorder.GetStateKeys()
// 			require.NoError(New(0).NewView(stateKeys, immutableScopeStorage(existingKeyValue).duplicate(), blockHeight).Remove(context.Background(), nonExistingKeys[keyIdx]))
...
// 			// validate operation agaist TStateView
// 			stateKeys := recorder.GetStateKeys()
// 			stateValue, err := New(0).NewView(stateKeys, immutableScopeStorage(existingKeyValue), blockHeight).GetValue(context.Background(), existingKeys[keyIdx])
// 			require.NoError(err)
...
// 			// validate operation agaist TStateView
// 			stateKeys := recorder.GetStateKeys()
// 			_, err = New(0).NewView(stateKeys, immutableScopeStorage(existingKeyValue), blockHeight).GetValue(context.Background(), nonExistingKeys[keyIdx])

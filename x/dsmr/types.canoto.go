...
const (
	canoto__Chunk__Builder__tag = "\x0a" // canoto.Tag(1, canoto.Len)
	canoto__Chunk__Expiry__tag  = "\x10" // canoto.Tag(2, canoto.Varint)
	canoto__Chunk__Data__tag    = "\x1a" // canoto.Tag(3, canoto.Len)
)
...
func (c *Chunk) CalculateCanotoCache() {
	...
	c.canotoData.size = 0
	if !canoto.IsZero(c.Builder) {
		c.canotoData.size += len(canoto__Chunk__Builder__tag) + canoto.SizeBytes(c.Builder[:])
	}
	if !canoto.IsZero(c.Expiry) {
		c.canotoData.size += len(canoto__Chunk__Expiry__tag) + canoto.SizeInt(c.Expiry)
	}
	if len(c.Data) != 0 {
		c.canotoData.size += len(canoto__Chunk__Data__tag) + canoto.SizeBytes(c.Data)
	}
}
...
func (c *Chunk) MarshalCanotoInto(w canoto.Writer) canoto.Writer {
	...
	if !canoto.IsZero(c.Builder) {
		canoto.Append(&w, canoto__Chunk__Builder__tag)
		canoto.AppendBytes(&w, c.Builder[:])
	}
	if !canoto.IsZero(c.Expiry) {
		canoto.Append(&w, canoto__Chunk__Expiry__tag)
		canoto.AppendInt(&w, c.Expiry)
	}
	if len(c.Data) != 0 {
		canoto.Append(&w, canoto__Chunk__Data__tag)
...
func (c *SerializeChunkCertificate) UnmarshalCanotoFrom(r canoto.Reader) error {
	...
	for canoto.HasNext(&r) {
		...
		case 1:
			...
			remainingBytes := r.B
			r.B = msgBytes
			c.Reference = canoto.MakePointer(c.Reference)
...
func (c *SerializeChunkCertificate) ValidCanoto() bool {
	...
	if c.Reference != nil && !(c.Reference).ValidCanoto() {
		return false
	}
	return true
}
...
func (c *Block) UnmarshalCanotoFrom(r canoto.Reader) error {
	...
	for canoto.HasNext(&r) {
		...
		case 4:
			...
			if len(msgBytes) != 0 {
				r.B = msgBytes
				c.Chunks[0] = canoto.MakePointer(c.Chunks[0])
			...
			for i := range count {
				...
				remainingBytes := r.B
				r.B = msgBytes
				c.Chunks[1+i] = canoto.MakePointer(c.Chunks[1+i])
...
func (c *Block) ValidCanoto() bool {
	...
	for i := range c.Chunks {
		if c.Chunks[i] != nil && !(c.Chunks[i]).ValidCanoto() {
			return false
		}
	}

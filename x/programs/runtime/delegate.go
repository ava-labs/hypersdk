...
func getCallArgs(ctx context.Context, runtime Runtime, buffer []byte, delegateProgramID uint64) ([]uint64, error) {
	...
	for !p.Empty() {
		...
		isInt := p.UnpackBool()
		if isInt {
			value_int := p.UnpackUint64(true)
			args = append(args, value_int)
		} else {
			value_bytes := make([]byte, size)
			p.UnpackFixedBytes(int(size), &value_bytes)
			// Write to memory using alloc

			ptr, err := runtime.WriteGuestBuffer(ctx, value_bytes)
			if err != nil {
				return nil, err
			}

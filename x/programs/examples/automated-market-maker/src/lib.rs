...
pub fn init(context: &mut Context, token_x: Address, token_y: Address, liquidity_token: ProgramId) {
    ...
    let liquidity_context = ExternalCallContext::new(context, lt_program, MAX_GAS, 0);
    token::init(
        &liquidity_context,
...
pub fn remove_liquidity(context: &mut Context, shares: Units) -> (Units, Units) {
    ...
    let actor_total_shares = token::balance_of(&lp_token, lp_token.get_context().actor());
...
fn external_token_contracts(context: &mut Context) -> (ExternalCallContext, ExternalCallContext) {
    ...
    (
        ExternalCallContext::new(context, token_x, MAX_GAS, 0),
        ExternalCallContext::new(context, token_y, MAX_GAS, 0),
    )
}
...
fn external_liquidity_token(context: &mut Context) -> ExternalCallContext {
    ...
    ExternalCallContext::new(context, token, MAX_GAS, 0)
}

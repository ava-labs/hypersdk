...
use wasmlanche::{borsh::{BorshDeserialize, BorshSerialize}, Address, ExternalCallError};
...
fn amm_init() {
    ...
    context.mock_call_function(
        alice,
        "init",
        (String::from("liquidity token"), String::from("LT")),
        MAX_GAS,
        0,
        NoReturn::Ok(()),
    );

    init(&mut context, token_x, token_y, liquidity_id);
}

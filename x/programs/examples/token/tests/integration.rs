...
fn init_token() {
    ...
    let program_address = simulator.create_contract(PROGRAM_PATH).unwrap().address;

    simulator
        .call_contract::<(), _>(program_address, "init", ("Test", "TST"), MAX_UNITS)
        .unwrap();

    let supply = simulator
        .call_contract::<Units, _>(program_address, "total_supply", (), MAX_UNITS)
        .unwrap();
    assert_eq!(supply, 0);

    let symbol = simulator
        .call_contract::<String, _>(program_address, "symbol", (), MAX_UNITS)
        .unwrap();
    assert_eq!(symbol, "TST");

    let name = simulator
        .call_contract::<String, _>(program_address, "name", (), MAX_UNITS)
        .unwrap();
    assert_eq!(name, "Test");
}
...
fn mint() {
    ...
    let balance: Units = simulator
        .call_contract(program_address, "balance_of", (alice,), MAX_UNITS)
        .unwrap();
    assert_eq!(balance, alice_initial_balance);

    let total_supply: Units = simulator
        .call_contract(program_address, "total_supply", (), MAX_UNITS)
        .unwrap();

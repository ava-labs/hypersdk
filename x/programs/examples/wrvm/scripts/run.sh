...
go build -v -o ${TMPDIR}/wr-cli ./cmd/wr-cli
...
[{"address":"wr1rvzhmceq997zntgvravfagsks6w0ryud3rylh4cdvayry0dl97nsp30ucp", "balance":1000000000000}]
EOF

GENESIS_PATH=$2
if [[ -z "${GENESIS_PATH}" ]]; then
  echo "creating VM genesis file with allocations"
  rm -f ${TMPDIR}/wrvm.genesis
  ${TMPDIR}/wr-cli genesis generate ${TMPDIR}/allocations.json \
  --max-block-units 4000000 \
  --window-target-units 100000000000 \
  --min-block-gap ${MIN_BLOCK_GAP} \
  --genesis-file ${TMPDIR}/wrvm.genesis
else
  echo "copying custom genesis file"
  rm -f ${TMPDIR}/wrvm.genesis
  cp ${GENESIS_PATH} ${TMPDIR}/wrvm.genesis
fi
...
rm -f ${TMPDIR}/wrvm.config
rm -rf ${TMPDIR}/wrvm-e2e-profiles
cat <<EOF > ${TMPDIR}/wrvm.config
{
  "mempoolSize": 10000000,
  "mempoolPayerSize": 10000000,
  "mempoolExemptPayers":["wr1rvzhmceq997zntgvravfagsks6w0ryud3rylh4cdvayry0dl97nsp30ucp"],
  "parallelism": 5,
...
EOF
mkdir -p ${TMPDIR}/wrvm-e2e-profiles

# Profiling config:  {"continuousProfilerDir":"${TMPDIR}/wrvm-e2e-profiles/*"}
...
rm -f ${TMPDIR}/wrvm.subnet
cat <<EOF > ${TMPDIR}/wrvm.subnet
{
...
--vm-genesis-path=${TMPDIR}/wrvm.genesis \
--vm-config-path=${TMPDIR}/wrvm.config \
--subnet-config-path=${TMPDIR}/wrvm.subnet \

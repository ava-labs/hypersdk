...
pub fn simple_call_external(ctx: &mut Context, target: Address, max_units: Gas) -> i64 {
    ctx
        .call_function(target, "simple_call", &[], max_units, 0)
        .unwrap()
}
...
pub fn actor_check_external(ctx: &mut Context, target: Address, max_units: Gas) -> Address {
    ctx
        .call_function(target, "actor_check", &[], max_units, 0)
        .expect("failure")
}
...
pub fn call_with_param_external(
    ...
    ctx
        .call_function(target, "call_with_param", &value.to_le_bytes(), max_units, 0)
        .unwrap()
}
...
pub fn call_with_two_params_external(
    ...
    let args: Vec<_> = value1
        ...
        .collect();
    ctx
        .call_function(target, "call_with_two_params", &args, max_units, 0)
        .unwrap()
}

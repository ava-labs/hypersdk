...
/// `RuntimePtr` is an i64 where the first 4 bytes represent the length of the bytes
/// and the following 4 bytes represent a pointer to WASM memeory where the bytes are stored.
pub type RuntimePtr = i64;
...
pub fn to_runtime_ptr(arg: &[u8]) -> Result<RuntimePtr, StateError> {
    ...
    let runtime_ptr = i64::from(ptr as u32) | (i64::from(len as u32) << 32);
    Ok(runtime_ptr)
}
...
pub fn split_runtime_ptr(arg: RuntimePtr) -> (i64, usize) {
    assert!(arg >= 0);
...
pub unsafe fn from_runtime_ptr<V>(ptr: RuntimePtr) -> Result<V, StateError>
where
    V: BorshDeserialize,
{
...
/// `runtime_ptr` is encoded using Big Endian as an i64.
#[must_use]
pub fn into_bytes(runtime_ptr: RuntimePtr) -> Vec<u8> {
    // grab length from ptrArg
    let (ptr, len) = split_runtime_ptr(runtime_ptr);

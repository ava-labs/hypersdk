...
extern "C" {
    #[link_name = "put"]
    fn _put(caller_id: i64, key: i64, value: i64) -> i32;

    #[link_name = "get"]
    fn _get(caller_id: i64, key_ptr: i64) -> i64;
}
    ...
    V: BorshSerialize,
{
    ...
    match unsafe { _put(caller.id(), key_bytes.as_ptr() as i64, value_bytes.as_ptr() as i64) } {
        0 => Ok(()),
...
pub(crate) unsafe fn get_bytes(caller: &Program, key: &Key) -> i64 {
    ...
    unsafe { _get(caller.id(), key.as_ptr() as i64) }
}

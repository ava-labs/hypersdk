use wasmlanche_sdk::{params,program::Program, context::Context, public, state_keys, types::Address};
...
#[public]
fn initialize_address(ctx: Context, address: Address) -> bool {
    if ctx.program
        .state()
...
fn inc(ctx: Context, to: Address, amount: i64) -> bool {
    let counter = amount + get_value(ctx, to);
...
#[public]
fn inc_external(_: Context, target: Program, max_units: i64, of: Address, amount: i64) -> i64 {
    target
...
#[public]
fn get_value(ctx: Context, of: Address) -> i64 {
    ctx.program
        .state()
...
#[public]
fn get_value_external(_: Context, target: Program, max_units: i64, of: Address) -> i64 {
    target

...
impl ProgramContext {
    ...
    pub fn store_map_value<T: Store>(
        ...
        store_key_value(self, key_bytes.as_bytes(), &value)
    }
...
fn get_field_as_bytes(ctx: &ProgramContext, name: &str) -> Result<Vec<u8>, StorageError> {
    ...
    let bytes_ptr = get_bytes(ctx, name_ptr, name_len, bytes_len) as *mut u8;
...
fn get_field<T: Store>(ctx: &ProgramContext, name: &str) -> Result<T, StorageError> {
    let bytes = get_field_as_bytes(ctx, name)?;
    T::from_bytes(&bytes)
}

/// Gets the correct key to in the host storage for a [map_name] and [key] within that map  
fn get_map_key(map_name: &str, key: &ProgramValue) -> String {
    let key_bytes = [map_name.as_bytes(), &key.as_bytes()].concat();
